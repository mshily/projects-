...
struct Node {
    ...
    std::shared_ptr<Node> zero_son;
    std::shared_ptr<Node> one_son;

    Node();
};

std::shared_ptr<Node> BuildTree(std::unordered_map<char16_t, int> weight);
...
struct Object {
    char16_t symbol;
    std::string code;
    Object(char16_t symbol_c, std::string code_c) {
        symbol = symbol_c;
        code = code_c;
    }
};
...
struct Compare {
    bool operator()(std::pair<int, std::shared_ptr<Node>> a, std::pair<int, std::shared_ptr<Node>> b);
...
void PrintCompressor(std::unordered_map<char16_t, std::string> &mp, const std::vector<Object> &codes, BitPrint &out,
                     const std::string &file_name, bool is_last_file, const std::string &file_path);