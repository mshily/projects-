...
size_t FindMaxCodeSize(const std::vector<SymbolCodes>& codes) {
    size_t mx = 0;
...
void SizeCounting(const std::vector<SymbolCodes>& codes, std::vector<int>& count_of_sizes) {
    for (const auto& code : codes) {
        ++count_of_sizes[code.code.size()];
    }
}

void PrintCompressor(std::unordered_map<char16_t, std::string>& math_of_codes, const std::vector<SymbolCodes>& codes,
